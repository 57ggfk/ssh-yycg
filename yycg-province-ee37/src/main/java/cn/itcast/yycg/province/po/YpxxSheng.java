package cn.itcast.yycg.province.po;

import java.util.Date;

public class YpxxSheng implements java.io.Serializable {

	private String id;
	private String bm;
	private String scqymc;
	private String spmc;
	private Float zbjg;
	private String zpdz;
	private String pzwh;
	private Date pzwhyxq;
	private String jky;
	private String bzcz;
	private String bzdw;
	private Float lsjg;
	private String lsjgcc;
	private String zlcc;
	private String zlccsm;
	private String ypjybg;
	private String ypjybgbm;
	private Date ypjybgyxq;
	private String cpsm;
	private String jyzt;
	private String vchar1;
	private String vchar2;
	private String vchar3;
	private String dw;
	private String mc;
	private String jx;
	private String gg;
	private String zhxs;
	private String py;
	private String lb;
	/**
	 * @return the id
	 */
	public String getId() {
		return id;
	}
	/**
	 * @param id the id to set
	 */
	public void setId(String id) {
		this.id = id;
	}
	/**
	 * @return the bm
	 */
	public String getBm() {
		return bm;
	}
	/**
	 * @param bm the bm to set
	 */
	public void setBm(String bm) {
		this.bm = bm;
	}
	/**
	 * @return the scqymc
	 */
	public String getScqymc() {
		return scqymc;
	}
	/**
	 * @param scqymc the scqymc to set
	 */
	public void setScqymc(String scqymc) {
		this.scqymc = scqymc;
	}
	/**
	 * @return the spmc
	 */
	public String getSpmc() {
		return spmc;
	}
	/**
	 * @param spmc the spmc to set
	 */
	public void setSpmc(String spmc) {
		this.spmc = spmc;
	}
	/**
	 * @return the zbjg
	 */
	public Float getZbjg() {
		return zbjg;
	}
	/**
	 * @param zbjg the zbjg to set
	 */
	public void setZbjg(Float zbjg) {
		this.zbjg = zbjg;
	}
	/**
	 * @return the zpdz
	 */
	public String getZpdz() {
		return zpdz;
	}
	/**
	 * @param zpdz the zpdz to set
	 */
	public void setZpdz(String zpdz) {
		this.zpdz = zpdz;
	}
	/**
	 * @return the pzwh
	 */
	public String getPzwh() {
		return pzwh;
	}
	/**
	 * @param pzwh the pzwh to set
	 */
	public void setPzwh(String pzwh) {
		this.pzwh = pzwh;
	}
	/**
	 * @return the pzwhyxq
	 */
	public Date getPzwhyxq() {
		return pzwhyxq;
	}
	/**
	 * @param pzwhyxq the pzwhyxq to set
	 */
	public void setPzwhyxq(Date pzwhyxq) {
		this.pzwhyxq = pzwhyxq;
	}
	/**
	 * @return the jky
	 */
	public String getJky() {
		return jky;
	}
	/**
	 * @param jky the jky to set
	 */
	public void setJky(String jky) {
		this.jky = jky;
	}
	/**
	 * @return the bzcz
	 */
	public String getBzcz() {
		return bzcz;
	}
	/**
	 * @param bzcz the bzcz to set
	 */
	public void setBzcz(String bzcz) {
		this.bzcz = bzcz;
	}
	/**
	 * @return the bzdw
	 */
	public String getBzdw() {
		return bzdw;
	}
	/**
	 * @param bzdw the bzdw to set
	 */
	public void setBzdw(String bzdw) {
		this.bzdw = bzdw;
	}
	/**
	 * @return the lsjg
	 */
	public Float getLsjg() {
		return lsjg;
	}
	/**
	 * @param lsjg the lsjg to set
	 */
	public void setLsjg(Float lsjg) {
		this.lsjg = lsjg;
	}
	/**
	 * @return the lsjgcc
	 */
	public String getLsjgcc() {
		return lsjgcc;
	}
	/**
	 * @param lsjgcc the lsjgcc to set
	 */
	public void setLsjgcc(String lsjgcc) {
		this.lsjgcc = lsjgcc;
	}
	/**
	 * @return the zlcc
	 */
	public String getZlcc() {
		return zlcc;
	}
	/**
	 * @param zlcc the zlcc to set
	 */
	public void setZlcc(String zlcc) {
		this.zlcc = zlcc;
	}
	/**
	 * @return the zlccsm
	 */
	public String getZlccsm() {
		return zlccsm;
	}
	/**
	 * @param zlccsm the zlccsm to set
	 */
	public void setZlccsm(String zlccsm) {
		this.zlccsm = zlccsm;
	}
	/**
	 * @return the ypjybg
	 */
	public String getYpjybg() {
		return ypjybg;
	}
	/**
	 * @param ypjybg the ypjybg to set
	 */
	public void setYpjybg(String ypjybg) {
		this.ypjybg = ypjybg;
	}
	/**
	 * @return the ypjybgbm
	 */
	public String getYpjybgbm() {
		return ypjybgbm;
	}
	/**
	 * @param ypjybgbm the ypjybgbm to set
	 */
	public void setYpjybgbm(String ypjybgbm) {
		this.ypjybgbm = ypjybgbm;
	}
	/**
	 * @return the ypjybgyxq
	 */
	public Date getYpjybgyxq() {
		return ypjybgyxq;
	}
	/**
	 * @param ypjybgyxq the ypjybgyxq to set
	 */
	public void setYpjybgyxq(Date ypjybgyxq) {
		this.ypjybgyxq = ypjybgyxq;
	}
	/**
	 * @return the cpsm
	 */
	public String getCpsm() {
		return cpsm;
	}
	/**
	 * @param cpsm the cpsm to set
	 */
	public void setCpsm(String cpsm) {
		this.cpsm = cpsm;
	}
	/**
	 * @return the jyzt
	 */
	public String getJyzt() {
		return jyzt;
	}
	/**
	 * @param jyzt the jyzt to set
	 */
	public void setJyzt(String jyzt) {
		this.jyzt = jyzt;
	}
	/**
	 * @return the vchar1
	 */
	public String getVchar1() {
		return vchar1;
	}
	/**
	 * @param vchar1 the vchar1 to set
	 */
	public void setVchar1(String vchar1) {
		this.vchar1 = vchar1;
	}
	/**
	 * @return the vchar2
	 */
	public String getVchar2() {
		return vchar2;
	}
	/**
	 * @param vchar2 the vchar2 to set
	 */
	public void setVchar2(String vchar2) {
		this.vchar2 = vchar2;
	}
	/**
	 * @return the vchar3
	 */
	public String getVchar3() {
		return vchar3;
	}
	/**
	 * @param vchar3 the vchar3 to set
	 */
	public void setVchar3(String vchar3) {
		this.vchar3 = vchar3;
	}
	/**
	 * @return the dw
	 */
	public String getDw() {
		return dw;
	}
	/**
	 * @param dw the dw to set
	 */
	public void setDw(String dw) {
		this.dw = dw;
	}
	/**
	 * @return the mc
	 */
	public String getMc() {
		return mc;
	}
	/**
	 * @param mc the mc to set
	 */
	public void setMc(String mc) {
		this.mc = mc;
	}
	/**
	 * @return the jx
	 */
	public String getJx() {
		return jx;
	}
	/**
	 * @param jx the jx to set
	 */
	public void setJx(String jx) {
		this.jx = jx;
	}
	/**
	 * @return the gg
	 */
	public String getGg() {
		return gg;
	}
	/**
	 * @param gg the gg to set
	 */
	public void setGg(String gg) {
		this.gg = gg;
	}
	/**
	 * @return the zhxs
	 */
	public String getZhxs() {
		return zhxs;
	}
	/**
	 * @param zhxs the zhxs to set
	 */
	public void setZhxs(String zhxs) {
		this.zhxs = zhxs;
	}
	/**
	 * @return the py
	 */
	public String getPy() {
		return py;
	}
	/**
	 * @param py the py to set
	 */
	public void setPy(String py) {
		this.py = py;
	}
	/**
	 * @return the lb
	 */
	public String getLb() {
		return lb;
	}
	/**
	 * @param lb the lb to set
	 */
	public void setLb(String lb) {
		this.lb = lb;
	}

}
